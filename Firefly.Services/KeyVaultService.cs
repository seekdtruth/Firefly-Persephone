...
namespace Firefly.Services
{
    ...
    public class KeyVaultService : IKeyVaultService
    {
        ...
        public KeyVaultService(IFireflyConfiguration configuration, ILoggerFactory? loggerFactory)
        {
            ...
            this.logger = loggerFactory.CreateLogger<KeyVaultService>();

            this.secretClient = new SecretClient(configuration.KeyVaultUri, configuration.KeyVaultCredential, new SecretClientOptions()
            {
                Retry =
                {
        ...
        public KeyVaultSecret GetSecret(string secretKey, CancellationToken cancellationToken = new CancellationToken())
        {
            try
            {
                ...
                if (response == null) throw new ApplicationException($"Unable to retrieve secret: {secretKey}");
